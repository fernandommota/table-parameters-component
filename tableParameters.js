...
var TableParameters = UnmanagedComponent.extend({
  ...
  update: function() {
    ...
    if(!this.htmlObject) {
      return this.error("TableParameters requires an htmlObject");
    }
    try{
      this.block();
      this.setup();
      if(this.chartDefinition.paginateServerside) {
        ...
        var success = _.bind(function(data){
          this.rawData = data;
          this.processTableParametersResponse(data)
        },this);
  ...
  paginatingUpdate: function() {
    ...
    this.queryState.setCallback(_.bind(function(values) {
      ...
      this.processTableParametersResponse(values);
    },this));
    this.queryState.setParameters(this.parameters);
    this.queryState.setAjaxOptions({async:true});
    this.processTableParametersResponse();
   },
  ...
  processTableParametersResponse : function(json) {
    var myself = this,
        cd = this.chartDefinition,
        extraOptions = {};

    this.ph.trigger('cdfTableParametersProcessResponse');
    ...
    var dtData0 = TableParameters.getDataTableOptions(cd);
    ...
    for(var i=0; i< this.parameters.length; i++){
    	var nameParameter=this.parameters[i][0];
    	arrayParameters[i]=[ nameParameter,Dashboards.getParameterValue(nameParameter)]; 
    } 

    dtData.aoColumns=aoColumns;
	dtData.aaData=arrayParameters;
	
	/****                   Parameters                                     ******/

    this.ph.html("<table id='" + this.htmlObject + "Table' class='TableParameters' width='100%'></table>");
    /* 
    ...
    myself.ph.trigger('cdfTableParametersFinishRendering');
  },
